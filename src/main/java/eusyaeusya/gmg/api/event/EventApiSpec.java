...
public interface EventApiSpec {
        @Operation(summary = "새로운 이벤트 생성", description = "이벤트 생성 요청(EventCreateRequest)을 받아 새로운 이벤트를 생성합니다. 성공 시 생성된 이벤트 정보를 반환합니다.")
        @PostMapping
        ApiResponse<EventCreateResponse> createEvent(@Valid @RequestBody EventCreateRequest request);

        @Operation(summary = "이벤트의 선택 가능한 카테고리 조회", description = """
        ...
        ApiResponse<EventPlaceTypeCategoriesResponse> getAvailableCategoriesForEvent(
                        @Parameter(description = "이벤트 해시 URL", example = "abc123") @PathVariable String hashUrl);

        @Operation(summary = "이벤트 메인 페이지 정보 조회", description = """
                        초기 히트맵 데이터를 포함한 모임 정보를 조회합니다
                        """)
        @GetMapping("/{hashUrl}")
        ApiResponse<EventMainResponse> getEventMain(@PathVariable String hashUrl);

        @Operation(summary = "히트맵 실시간 스트림 구독", description = """
        ...
        ResponseEntity<SseEmitter> streamHeatmap(@PathVariable String hashUrl);
}
